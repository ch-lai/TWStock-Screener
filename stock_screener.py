...
class StockScreener:
    def __init__(self, n_months=12, load_report=True, save_report=False):
        # 獲取近n個月的營收資料
        self.data_m = crawl_report.cast_n_months(
            n_months=12, load_report=True, save_report=False)
        # 數據處理─結合月營收資訊
        self.df_m = pd.DataFrame(
            {k: df['當月營收'] for k, df in self.data_m.items()}).transpose().sort_index()

        # 獲取近n個季的財報資料
        self.data_s = crawl_report.cast_n_seasons(
            n_seasons=2, load_report=True, save_report=False)
        # 數據處理─結合季毛利率資訊
    ...
    def GPM_growth(self):
        # 毛利率成長法選股
        cond = self.df_gpm.iloc[-1] == self.df_gpm.iloc[-2:].max()
        self.mask_GPM_growth = np.asarray(cond)

    def OPM_growth(self):
        # 營業利益率成長法選股
        cond = self.df_opm.iloc[-1] == self.df_opm.iloc[-2:].max()
        self.mask_OPM_growth = np.asarray(cond)

    def NPM_growth(self):
        # 稅後純益率成長法選股
        cond = self.df_npm.iloc[-1] == self.df_npm.iloc[-2:].max()
        self.mask_NPM_growth = np.asarray(cond)
    ...
    def Filter_stocks(self):
        ...
        # # NOTE: havent merge
        # self.mask = self.mask_GPM_growth & self.mask_OPM_growth & self.mask_NPM_growth
        # idx = np.where(self.mask > 0)
        ...
        # argsort = np.flip(np.squeeze(np.argsort(score, axis=1)))
        # # 只保留 [公司代號, 公司名稱]
        # result = self.data_s[list(self.data_s)[0]].to_numpy()[idx, :2]
